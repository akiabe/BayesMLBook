...
# ╔═╡ 8b1c24d3-ff22-4bc3-8121-30482559092a
begin
	using Distributions
	using Plots
	using StatsPlots
...
# ╔═╡ b3082e0d-c69d-4e52-b50b-a9da607e4b66
begin
	using Turing
	using MCMCChains
end

# ╔═╡ ee2f5560-4510-4e2c-b564-906d80d7cd8d
md"""
Turing.jiを使って、ガウス混合モデルの推論を動かしてみる。
https://turing.ml/dev/tutorials/01-gaussian-mixture-model/
"""

# ╔═╡ 60fe0aa8-db7d-43fd-8dbf-fba117c4e840
begin
	# ガウス混合分布のデータを生成
	μ1 = [-1.0, 1.0]
	Σ1 = [
	...
	scatter(X[1, :], X[2, :], xlab="x1", ylab="x2", label=:none)
end

# ╔═╡ 45a60e63-5004-41c4-a37d-034b513d06c1
# モデル
@model function gaussian_mix_model(X)
    μ1 ~ Normal()
    μ2 ~ Normal()
	μ = [μ1, μ2]
	p = [0.5, 0.5]
    ...
    for i in 1:N
        S[i] ~ Categorical(p)
		X[:, i] ~ MvNormal([μ[S[i]], μ[S[i]]], 1.)
    end
    return S
end;

# ╔═╡ d9254143-80e0-450a-9d95-e1882707ee91
gmm_model = gaussian_mix_model(X);

# ╔═╡ 4e5c94d1-f018-447b-a93c-0e53854ca5a2
gmm_sampler = Gibbs(PG(100, :k), HMC(0.05, 10, :μ1, :μ2));

# ╔═╡ 7acd9601-5609-426f-90c3-cebe8691807c
chain = sample(gmm_model, gmm_sampler, MCMCThreads(), 100, 3);

# ╔═╡ 124f8488-8554-4712-914c-eba9cba1e47a
begin
	ids = findall(map(name -> occursin("μ", string(name)), names(chain)));
	plot(chain[:, ids, :]; legend=true, labels=["Mu 1" "Mu 2"], colordim=:parameter)
end

# ╔═╡ 5fc6f68d-0622-44c4-87fa-34d3b44f4c6a
chain[:, :, 1]

# ╔═╡ 3c877280-182c-4a3a-bf66-004dd0f2ac8e
...
# ╔═╡ 84154b75-8db1-4b2d-991a-e3d322f3c1ce
contour(range(-5, stop=3), range(-6, stop=2),
    (x, y) -> predict(x, y, [0.5, 0.5], [mean(chain[:μ1]), mean(chain[:μ2])]))
scatter!(X[1, :], X[2, :]; legend=false, title="Synthetic Dataset")

# ╔═╡ 7d416ce4-7ef3-4169-9349-8538307c4b3a
